...
UAsyncRootMovement* UAsyncRootMovement::AsyncRootMovement(const UObject* WorldContext, UCharacterMovementComponent* CharacterMovement, FAsyncRootMovementParams Params)
{
	UWorld* ContextWorld = GEngine->GetWorldFromContextObject(WorldContext, EGetWorldErrorMode::ReturnNull);
	if (!ensureAlwaysMsgf(IsValid(WorldContext), TEXT("World Context was not valid.")))
	{
	...
	Node->ContextWorld = ContextWorld;
	Node->CharacterMovement = CharacterMovement;
	Node->WorldDirection = Params.WorldDirection;
	Node->Strength = Params.Strength;
	Node->Duration = Params.Duration;
	Node->bIsAdditive = Params.bIsAdditive;
	Node->StrengthOverTime = Params.StrengthOverTime;
	Node->VelocityOnFinishMode = Params.VelocityOnFinishMode;
	Node->SetVelocityOnFinish = Params.SetVelocityOnFinish;
	Node->ClampVelocityOnFinish = Params.ClampVelocityOnFinish;
	Node->bEnableGravity = Params.bEnableGravity;
	Node->RegisterWithGameInstance(ContextWorld->GetGameInstance());
	return Node;
}
